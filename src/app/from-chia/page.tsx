...
export default function FromChia() {
  ...
  const createSpendBundle = async () => {
    ...
    const sb = await burnCATs(
      burnSendCoinParent,
      Math.ceil(parseFloat(tokenAmountStr) * 1000),
      burnSendCoinParentSpend,
      offer,
      "657468", // eth
      ethTokenAddress,
      ethAddress
  ...
  return (
    <main className="flex min-h-screen flex-col items-center pr-48 pl-48 pt-16 pb-8">
      ...
      <div className="flex flex-col space-y-4 w-full pb-16">
        <label className="text-lg font-semibold">2. Initialize BLS Module</label>
      ...
      <div className="flex flex-col space-y-4 w-full pb-16">
        <label className="text-lg font-semibold">3. Upload offer</label>
